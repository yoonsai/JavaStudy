...
/*
 *  접근지정어
 *  => 어느 클래스까지 접글할 수 있는지 설정
 *  private : 자신의 클래스에서 사용이 가능
 *            => 멤버변수 (저장된 데이터 보호)
 *  default : 같은 패키지안에서만 접근이 가능 (아무것도 안붙어있으면 default) 
 *            => 사용빈도가 거의 없다                                      
 *  protected : 같은 패키지안에서 접근이 가능
 *              상속을 받은 경우엔 상속내린 클래스의 패키지까지 접근이 가능
 *              => 사용빈도가 거의 없다
 ...
 *      --------- private
 *                   |
 *                기본형(int,long..) / 참조형(사용자 정의 => 배열,클래스)
 ...
 *     => 패키지 제작 : 라이브러리
 *     => 패키지를 사용하면
 *        같은 패키지 => import없이 사용이 가능
 *        다른 패키지 => import를 사용해서 클래스를 읽어온다
 *                   -------
 *                   라이브러리 / 사용자 정의 클래스
