...
public class MusicSystem {
	@Getter
	@Setter
	static Music[] musics=new Music[50]; //데이터목록(배열 등) => static
    static // static블록 초기화 (자동으로 초기화할때)
	{
		// 초기화 블록 => static변수만 사용이 가능 => 자동 인식
    	// 초기화 블록은 상속이 불가능 => 호출 없이 실행 가능
    	// 생성자를 이용할 수 있다
    	// 1. 예외처리 => io(파일,메모리,네트워크) => 프로그램 정상종료
