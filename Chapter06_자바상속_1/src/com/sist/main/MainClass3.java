 ...
 *  객체 지향
 *  --------
 *    데이터 손실 방지 : 캡슐화 (변수:private, 기능 => 처리)
 *                        --- 저장 / 읽기 => getter/setter
 *                        --- HTML(Jsoup)
 *    재사용 : 상속 / 포함 (***)
 *    수정 / 추가 : 유지보수
 ...
 *        class Window
 *        {
 *           JFrame fr=new JFrame() => ***실제로 이렇게 많이 사용 //spring은 상속을 권장하지않음
 *        }
 *        ---> 변경해서 사용 => 상속
 ...
 *      class A
 *      class C
 *      class B extends A,C => 오류 (C언어, React) => 단일상속만 가능
 *      -----------------------------------------
 *      class A
 *      class C extends A => A,C
 *      class B extends C => A,C,B
 *      => 상속을 받은 클래스가 메모리는 크다
 *      => 클래스의 크기는 위로 올라갈수록 크다 (범위) | "이다" => 상위=공통기반 => 위에는 추상화, 아래는 구체화
 *      
 *           생물
 *            |
 *         --------
